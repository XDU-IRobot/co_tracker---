...
#预训练权重的检查点（checkpoint）、是否离线工作（offline）、是否使用v2版本（v2）以及窗口长度（window_len)
class CoTrackerPredictor(torch.nn.Module):
    ...
    def forward(
        ...
        queries: torch.Tensor = None, #需要查询的点
        segm_mask: torch.Tensor = None,  # Segmentation mask of shape (B, 1, H, W) 分割掩码
        grid_size: int = 0,  #密集追踪，就算没有提供点的时候，他会生成一张网格，分布在视频的第一帧，然后在后续帧中追踪这些点
    ...
    #这里是反向跟踪的函数，因为前向跟踪有时候会丢失目标，所以通过反向跟踪来补充
    #反向跟踪的原理是将视频序列倒过来，然后对查询点的x坐标进行调整，使其对应于倒置后的视频帧
    #然后再进行跟踪，最后将结果再倒过来
    #在反向跟踪中，我们只更新那些在前向跟踪中未被跟踪到的点
    #通过这种方式，我们可以提高整体的跟踪效果
